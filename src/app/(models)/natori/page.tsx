...
// Tipos de animação do Natori
const NATORI_ANIMATIONS = {
  motions: {
    description: "Animações de movimento do Natori",
    priority: 1,
    autoPlay: true,
  },
};

export default function NatoriPage() {
    ...
    try {
      ...
      if (motionGroups.includes(groupName)) {
        ...
        if (motionIndex < motionCount) {
          const animationInfo = NATORI_ANIMATIONS[groupName as keyof typeof NATORI_ANIMATIONS];
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setStatus("Carregando modelo Natori...");

        // Cria o modelo Live2D
        const model = await Live2DModel.from("/models/Natori/Natori.model3.json");
        ...
        addAnimationLog("✅ Modelo Natori carregado com sucesso!");
  ...
  return (
    <Live2DPageLayout title="🎭 Natori Live2D">
      {/* Painel de controles, canvas, logs, etc. */}
      {/* Seta de Voltar */}

      {/* Painel de Controles */}
      <Live2DPanel title="🎭 Natori Live2D">
        ...
        <Live2DSection title="🎮 Controles">
          <Live2DLabel>🎮 Controles</Live2DLabel>
        ...
        <Live2DSection title="📋 Logs de Animação">
          <Live2DLabel>📋 Logs de Animação</Live2DLabel>
          <Live2DLog logs={animationLog} />
        </Live2DSection>

        <Live2DSection title="🎭 Seletor Específico">
          <Live2DLabel>🎭 Seletor Específico</Live2DLabel>
        ...
        <Live2DSection title="🎛️ Controles de Escala">
          <Live2DLabel>
        ...
        <Live2DSection title="📊 Status">
          <Live2DLabel>📊 Status</Live2DLabel>
          <div style={{ fontSize: "12px", color: "#aaa" }}>{status}</div>
        </Live2DSection>

        <Live2DSection title="ℹ️ Informações">
          <Live2DLabel>ℹ️ Informações</Live2DLabel>
          <p>
            🛌 <strong>Idle:</strong> Animações de repouso/espera (padrão)
          </p>
          <p>
            🎭 <strong>Motion:</strong> Animações de movimento geral
          </p>
          <p>
