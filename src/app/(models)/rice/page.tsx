...
// Tipos de animação do Rice
const RICE_ANIMATIONS = {
  idle: {
...
export default function RicePage() {
    ...
    try {
      ...
      if (motionGroups.includes(groupName)) {
        ...
        if (motionIndex < motionCount) {
          const animationInfo = RICE_ANIMATIONS[groupName as keyof typeof RICE_ANIMATIONS];
  ...
  const playSmartAnimation = (model: any, context: "idle" | "motion" = "idle") => {
    ...
    try {
      ...
      const animationInfo = RICE_ANIMATIONS[selectedGroup as keyof typeof RICE_ANIMATIONS];
      const description = animationInfo ? animationInfo.description : "Animação desconhecida";
  ...
  const startAutoAnimation = (model: any) => {
    ...
    animationIntervalRef.current = setInterval(() => {
      // 80% chance de idle, 20% chance de motion
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        // Remover função loadCubismRuntimes e chamadas a ela
        ...
        setStatus("Carregando modelo Rice...");

        // REGISTRA O TICKER - ESSENCIAL PARA ANIMAÇÕES
        Live2DModel.registerTicker(Ticker);

        // Cria o modelo Live2D
        const model = await Live2DModel.from("/models/Rice/Rice.model3.json");
        ...
        addAnimationLog("✅ Modelo Rice carregado com sucesso!");
  ...
  return (
    <Live2DPageLayout title="🍚 Rice Live2D">
      {/* Painel de controles, canvas, logs, etc. */}
      <Live2DPanel title="🍚 Rice Live2D">
