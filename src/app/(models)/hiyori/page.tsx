...
import { PIXI_LIVE2D_CONFIG } from "@/lib/pixi-live2d-config";
...
// Tipos de animação da Hiyori
const HIYORI_ANIMATIONS = {
  Idle: {
  ...
  Flick: {
    description: "Animação de movimento rápido (flick)",
    priority: 2,
    autoPlay: false,
  },
  FlickDown: {
    description: "Animação de movimento rápido para baixo",
    priority: 2,
    autoPlay: false,
  },
  Tap: {
    description: "Animação de toque/clique geral",
    priority: 3,
    autoPlay: false,
  },
  "Tap@Body": {
    description: "Animação de toque no corpo",
    priority: 3,
    autoPlay: false,
  },
  "Flick@Body": {
    description: "Animação de movimento rápido no corpo",
    priority: 2,
    autoPlay: false,
  },
};

export default function HiyoriPage() {
    ...
    try {
      ...
      if (motionGroups.includes(groupName)) {
        ...
        if (motionIndex < motionCount) {
          const animationInfo = HIYORI_ANIMATIONS[groupName as keyof typeof HIYORI_ANIMATIONS];
  ...
  const playSmartAnimation = (model: any, context: "idle" | "click" | "body" | "flick" = "idle") => {
    if (!model || !model.internalModel || !model.internalModel.motionManager) {
      addAnimationLog("❌ Modelo não está pronto para animação");
      return;
    }
    ...
    try {
      ...
      switch (context) {
        case "idle":
          // Prioriza Idle, mas pode usar outros se não houver
          if (motionGroups.includes("Idle")) {
            ...
            // Fallback para qualquer grupo disponível
        ...
        case "click":
          // Prioriza Tap, depois Tap@Body
          if (motionGroups.includes("Tap")) {
            selectedGroup = "Tap";
          } else if (motionGroups.includes("Tap@Body")) {
            selectedGroup = "Tap@Body";
          } else {
            selectedGroup = motionGroups[0];
          }
          const groupCount = model.internalModel.motionManager.definitions[selectedGroup]?.length || 0;
        ...
        case "body":
          // Prioriza animações relacionadas ao corpo
          if (motionGroups.includes("Tap@Body")) {
            selectedGroup = "Tap@Body";
          } else if (motionGroups.includes("Flick@Body")) {
            selectedGroup = "Flick@Body";
          } else if (motionGroups.includes("Tap")) {
            selectedGroup = "Tap";
          } else {
            selectedGroup = motionGroups[0];
          }
          const bodyGroupCount = model.internalModel.motionManager.definitions[selectedGroup]?.length || 0;
          selectedIndex = Math.floor(Math.random() * bodyGroupCount);
          break;

        case "flick":
          // Prioriza animações de flick
          if (motionGroups.includes("Flick")) {
            selectedGroup = "Flick";
          } else if (motionGroups.includes("FlickDown")) {
            selectedGroup = "FlickDown";
          } else if (motionGroups.includes("Flick@Body")) {
            selectedGroup = "Flick@Body";
          } else {
            selectedGroup = motionGroups[0];
          }
          const flickGroupCount = model.internalModel.motionManager.definitions[selectedGroup]?.length || 0;
          selectedIndex = Math.floor(Math.random() * flickGroupCount);
          break;
      }

      const animationInfo = HIYORI_ANIMATIONS[selectedGroup as keyof typeof HIYORI_ANIMATIONS];
      const description = animationInfo ? animationInfo.description : "Animação desconhecida";
  ...
  const startAutoAnimation = (model: any) => {
    ...
    animationIntervalRef.current = setInterval(() => {
      // 70% chance de idle, 30% chance de outras animações
      const random = Math.random();
      let context: "idle" | "click" | "body" | "flick" = "idle";
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        setStatus("Carregando modelo Hiyori...");

        // Cria o modelo Live2D
        const model = await Live2DModel.from(PIXI_LIVE2D_CONFIG.MODEL_CONFIG.HIYORI_PATH);
        ...
        model.on("pointertap", (_event: any) => {
          ...
          // Determina se foi clique no corpo baseado na posição
          const localPoint = _event.data.getLocalPosition(model);
          const isBodyClick = localPoint.y > 0; // Simplificado - pode ser melhorado

          if (isBodyClick) {
            playSmartAnimation(model, "body");
          } else {
            playSmartAnimation(model, "click");
          }
        });
        ...
        addAnimationLog("✅ Modelo Hiyori carregado com sucesso!");
  ...
  return (
    <Live2DPageLayout title="🎭 Hiyori Live2D">
      {/* Painel de controles, canvas, logs, etc. */}
      <Live2DPanel title="🎭 Hiyori Live2D">
        <Live2DSection title="🎭 Hiyori Live2D">
        ...
        <Live2DSection title="🎮 Controles Rápidos">
          <div className="quick-controls-grid">
            ...
            <Live2DButton
              onClick={() => {
                if (live2dModel) {
                  playSmartAnimation(live2dModel as any, "click");
                }
              }}
            >
              👆 Tap
            ...
            <Live2DButton
              onClick={() => {
                if (live2dModel) {
                  playSmartAnimation(live2dModel as any, "body");
                }
              }}
            >
              🎯 Body
            ...
            <Live2DButton
              onClick={() => {
                if (live2dModel) {
                  playSmartAnimation(live2dModel as any, "flick");
                }
              }}
            >
              💨 Flick
          ...
          <div className="auto-animation-control">
            <Live2DButton
              onClick={() => {
                if (animationIntervalRef.current) {
                  stopAutoAnimation();
          ...
          {motionGroups.length > 0 && (
            <>
              ...
              <select
                ...
                className="motion-group-selector"
              >
              ...
              {selectedMotionGroup && motionGroups.find((g) => g.name === selectedMotionGroup) && (
                <>
                  ...
                  <select
                    ...
                    className="motion-specific-selector"
                  >
                    {motionGroups
                  ...
                  <div className="motion-group-description">{motionGroups.find((g) => g.name === selectedMotionGroup)?.description}</div>
        ...
        <Live2DSection title="🎭 Hiyori Animations">
          <p>
            🛌 <strong>Idle:</strong> Repouso/espera (padrão)
          </p>
          <p>
            👆 <strong>Tap:</strong> Toque/clique geral
          </p>
          <p>
            🎯 <strong>Tap@Body:</strong> Toque no corpo
          </p>
          <p>
            💨 <strong>Flick:</strong> Movimento rápido
          </p>
          <p>
            💨 <strong>FlickDown:</strong> Movimento para baixo
          </p>
          <p>
            💨 <strong>Flick@Body:</strong> Movimento no corpo
          </p>
          <p>
